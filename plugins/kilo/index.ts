/**
 * memsearch Kilo Code plugin — semantic memory search across sessions.
 *
 * Registers:
 * - memory_search tool: semantic search over past memories
 * - memory_get tool: expand a chunk to full context
 * - memory_transcript tool: parse original conversation from Kilo Code SQLite
 ...
 * that polls the Kilo Code SQLite database for completed turns.
 */
 ...
 * The daemon polls Kilo Code's SQLite for completed turns and writes to daily .md files.
 */
  ...
  return {
    ...
    tool: {
      ...
      memory_transcript: tool({
        description:
          "Retrieve the original conversation from a past Kilo Code session. " +
