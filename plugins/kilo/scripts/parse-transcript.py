#!/usr/bin/env python3
"""Parse Kilo Code SQLite transcript for a given session.
...
The script reads from the Kilo Code SQLite database at:
...
def parse_session(session_id, limit=20, last_turn=False):
    ...
    if not os.path.exists(db_path):
        print(f"Error: Kilo Code database not found at {db_path}", file=sys.stderr)
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(description="Parse Kilo Code session transcript")
