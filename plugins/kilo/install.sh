#!/bin/bash
# Install the memsearch Kilo Code plugin.
#
# This script:
# 1. Detects if memsearch is installed
# 2. Symlinks the plugin to Kilo Code's plugins directory
...
KILO_PLUGINS_DIR="${HOME}/.config/kilo/plugins"
AGENTS_SKILLS_DIR="${HOME}/.agents/skills"

echo "=== memsearch Kilo Code Plugin Installer ==="
echo ""
...
# 2. Symlink plugin to Kilo Code plugins directory
mkdir -p "${KILO_PLUGINS_DIR}"
PLUGIN_LINK="${KILO_PLUGINS_DIR}/memsearch.ts"
...
# 3. Symlink skill to ~/.agents/skills/ (Kilo Code-compatible)
...
if command -v npm &>/dev/null; then
  (cd "${SCRIPT_DIR}" && npm install --save-dev @opencode-ai/plugin 2>/dev/null) && echo "[OK] Dependencies installed" || echo "[WARN] npm install failed — plugin may still work if Kilo Code provides the SDK"
else
...
echo ""
echo "The plugin will be auto-loaded next time you start Kilo Code."
echo ""
echo "To verify, start Kilo Code and check if memory_search tool appears:"
echo "  kilo"
echo ""
echo "Optional: Add to kilo.json for npm-based install:"
echo '  "plugin": ["memsearch-kilo"]'
echo ""
