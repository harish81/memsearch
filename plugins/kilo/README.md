# memsearch Kilo Plugin

Semantic memory search for [Kilo](https://github.com/kilo-org/kilocode) — gives your AI assistant persistent memory across sessions with zero user intervention.
...
ln -sf /path/to/memsearch/plugins/kilo/index.ts ~/.config/kilo/plugins/memsearch.ts

# 2. Symlink the skill (optional, for !memory-recall)
mkdir -p ~/.agents/skills
ln -sf /path/to/memsearch/plugins/kilo/skills/memory-recall ~/.agents/skills/memory-recall
```
...
| `memory_transcript` | Read original conversation from Kilo SQLite DB. |
...
### 14:30
<!-- session:ses_abc123 db:~/.local/share/kilo/kilo.db -->
...
| Feature | Claude Code | Kilo | OpenClaw |
|---------|-------------|----------|----------|
| Session storage | JSONL | SQLite | JSONL |
| Hook system | Shell scripts | TypeScript hooks | JS API |
| Summarizer | claude -p --model haiku | kilo prompt | openclaw agent |
