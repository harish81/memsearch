#!/usr/bin/env python3
"""Background daemon that watches Kilo Code SQLite for completed turns and captures them.

Usage:
  capture-daemon.py <project_dir> <collection_name> [--memsearch-cmd CMD]

The daemon polls the Kilo Code SQLite database for new completed messages,
...
def get_small_model():
    """Read small_model from kilo.jsonc config (fallback to model, then empty)."""
    config_paths = [
...
def ensure_isolated_config():
    ...
    isolated = "/tmp/kilo-memsearch-summarize/kilo"
    os.makedirs(isolated, exist_ok=True)
    # Copy kilo.json (provider config) but NOT plugins/
    src = os.path.expanduser("~/.config/kilo/kilo.jsonc")
    dst = os.path.join(isolated, "kilo.jsonc")
    if os.path.exists(src) and not os.path.exists(dst):
        shutil.copy2(src, dst)
    return os.path.dirname(isolated)  # /tmp/kilo-memsearch-summarize


def summarize_with_llm(turn_text, small_model):
    """Summarize using kilo run in isolated env (no plugins -> no recursion)."""
    ...
    full_prompt = (
        ...
        f"Write 'User asked/did...' and 'Kilo Code replied/did...'. "
    ...
    try:
        ...
        # Extract bullet points (skip any kilo run header lines)
        lines = output.split("\n")
...
def get_db_path():
    """Find the Kilo Code SQLite database."""
    default = os.path.expanduser("~/.local/share/kilo/kilo.db")
    if os.path.exists(default):
        return default
    xdg_data = os.environ.get("XDG_DATA_HOME", "")
    if xdg_data:
        alt = os.path.join(xdg_data, "kilo", "kilo.db")
...
def main():
    parser = argparse.ArgumentParser(description="Capture daemon for Kilo Code sessions")
    ...
    if not os.path.exists(db_path):
        print(f"Kilo Code database not found at {db_path}", file=sys.stderr)
        sys.exit(1)
